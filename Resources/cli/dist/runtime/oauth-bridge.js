 ...
 */
export function deriveChromeId(manifest) {
...
function substituteExtId(filePath, chromeId) {
    if (!chromeId || !existsSync(filePath))
        return;
    const src = readFileSync(filePath, "utf-8");
    if (!src.includes(EXT_ID_PLACEHOLDER))
        return;
 ...
 */
export function applyOAuthBridge(stageDir, manifest, chromeId) {
    const notes = [];
    ...
    const alreadyBridged = manifest.content_scripts.some((cs) => Array.isArray(cs.js) && cs.js.includes(BRIDGE_PAGE));
    if (!alreadyBridged) {
 ...
 */
function injectPolyfillImport(stageDir, swRel) {
...
function addWebAccessible(manifest, resource, matches) {
    ...
    const looseStrings = raw.filter((e) => typeof e === "string");
    const war = raw.filter((e) => typeof e === "object" && e !== null);
    if (looseStrings.length > 0) {
        war.push({ resources: looseStrings, matches: ["<all_urls>"] });
    }
    const already = war.some((e) => Array.isArray(e.resources) && e.resources.includes(resource));
    if (!already)
