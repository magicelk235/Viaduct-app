 ...
 */
export function writePolyfill(targetDir) {
 ...
 */
export function wireCdpKeepalive(dir, manifest) {
...
export function shimSource(config = {}) {
...
export function writeShim(targetDir, config = {}) {
...
function walkHtmlFiles(dir, acc = []) {
    ...
    for (const entry of entries) {
        ...
        if (name === "node_modules" || name === ".git" || name.startsWith("__MACOSX"))
            continue;
 ...
 */
function pageHandlesDarkMode(dir, htmlFile, html) {
    const declaresTheme = (css) => /prefers-color-scheme/i.test(css) || /\bcolor-scheme\s*:/i.test(css);
    if (declaresTheme(html))
        return true;
    // Also honor a <meta name="color-scheme"> the page already ships.
    if (/<meta[^>]+name\s*=\s*["']color-scheme["']/i.test(html))
        return true;
    ...
    while ((m = linkRe.exec(html)) !== null) {
        ...
        try {
            if (declaresTheme(readFileSync(cssPath, "utf-8")))
                return true;
        }
        catch {
...
export function injectShimIntoHtmlPages(dir, polyfillFile) {
    ...
    for (const file of walkHtmlFiles(dir)) {
        ...
        const csTag = !html.includes(COLOR_SCHEME_MARKER) && !pageHandlesDarkMode(dir, file, html)
 ...
 */
export function injectPopupSizing(dir, popupFile, fullHeight = false) {
 ...
 */
function swNeedsModule(src) {
    if (/\bimport\s*\.\s*meta\b/.test(src))
        return true;
    if (/(^|[;}\n])\s*import\s*["'`{*]/.test(src))
        return true;
    if (/(^|[;}\n])\s*import\s+[\w$]+\s*(?:,\s*[{*]|from\b)/.test(src))
        return true;
    if (/(^|[;}\n])\s*export\s*(?:default\b|[{*]|(?:const|let|var|function|class|async)\b)/.test(src))
        return true;
    return false;
}
/**
 ...
 */
export function convertServiceWorkerToBackgroundPage(dir, manifest, polyfillFile) {
 ...
 */
export function wireActionClickBridge(dir, manifest) {
    ...
    if (current?.default_popup)
        return false;
    if (!backgroundRegistersActionOnClicked(dir, manifest))
        return false;
 ...
 */
function extractActionMessage(dir, manifest) {
    const files = [];
    const sw = manifest.background?.service_worker;
    if (typeof sw === "string")
        files.push(sw);
...
function parseCombo(key) {
    ...
    for (const part of key.split("+").map((s) => s.trim().toLowerCase())) {
        if (part === "command" || part === "cmd")
            combo.meta = true;
        else if (part === "ctrl" || part === "control" || part === "macctrl")
            combo.ctrl = true;
        else if (part === "shift")
            combo.shift = true;
            ...
            if (named.length === 1)
                combo.key = named;
            else if (COMMAND_KEY_TO_DOM[named])
                combo.key = COMMAND_KEY_TO_DOM[named];
            else
                unmappable = true; // a named key we can't match against e.key → bail
        }
    }
 ...
 */
export function wireActionHotkey(dir, manifest) {
    if (!backgroundRegistersActionOnClicked(dir, manifest))
        return null;
    ...
    const keyOf = (name) => {
        const sk = commands[name]?.suggested_key;
    ...
    let combo = preferred ? parseCombo(keyOf(preferred)) : null;
    let label = "";
    let usedCmd = null;
    if (combo && preferred) {
        label = String(keyOf(preferred));
        usedCmd = preferred;
    }
    if (!combo) {
        combo = { meta: false, ctrl: true, shift: true, alt: false, key: "y" };
        label = "Ctrl+Shift+Y";
    }
    if (usedCmd) {
...
`;
    ...
    for (const cs of manifest.content_scripts) {
        if (Array.isArray(cs.js) && cs.world !== "MAIN" && !cs.js.includes(ACTION_HOTKEY_FILENAME)) {
...
const PAGE_WORLD_INJECT_RE = /\.src\s*=\s*[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*\.runtime\.getURL\(\s*(["'])([^"'`]+\.m?js)\1\s*\)/g;
/**
 ...
 */
export function wireUserScriptsContentScript(dir, manifest, original = manifest) {
    const perms = [
...
`;
    ...
    const already = manifest.content_scripts.some((cs) => Array.isArray(cs.js) && cs.js.includes(USERSCRIPTS_CS_FILENAME));
    if (!already) {
...
export function wirePageWorldMainInjection(dir, manifest) {
    if (!Array.isArray(manifest.content_scripts))
        manifest.content_scripts = [];
    ...
    for (const target of targets) {
        ...
        const already = manifest.content_scripts.some((cs) => cs.world === "MAIN" && Array.isArray(cs.js) && cs.js.includes(target));
        if (already)
            continue;
        manifest.content_scripts.push({
            matches,
 ...
 */
function walkCode(src, start, onCode) {
        ...
        if (typeof top === "number") {
            if (c === "}") {
                if (top === 0) {
                    stack.pop();
                    continue;
                } // closes the ${ — resume template text
 ...
 */
export function matchBalancedParen(src, open) {
    let depth = 0;
...
function startsRegexAt(src, slashIdx, floor) {
    let j = slashIdx - 1;
    while (j >= floor && (src[j] === " " || src[j] === "\t" || src[j] === "\n" || src[j] === "\r"))
        j--;
    if (j < floor)
        return true; // expression start
    const c = src[j];
    if ("(,=:[!&|?{};+-*%^~<>".indexOf(c) >= 0)
        return true;
    if (/[A-Za-z0-9_$]/.test(c)) {
        let k = j;
        while (k >= floor && /[A-Za-z0-9_$]/.test(src[k]))
            k--;
...
function hoistImportScripts(dir, swPath) {
    const none = { tags: "", hoisted: new Set(), dynamic: false };
 ...
 */
function neutralizeImportScripts(dir, rootRel, resolveDir) {
    ...
    while ((mm = callOpenRe.exec(src))) {
        ...
        if (end < 0)
            continue; // unbalanced (shouldn't happen in valid JS) → leave as-is
        ...
        if (argList.replace(stripStatics, "").replace(stripComments, "").replace(/[\s,]/g, "") !== "")
            dynamic = true;
        ...
        while ((m = argRe.exec(argList))) {
            ...
            if (existsSync(join(dir, fromRoot)))
                targets.push(fromRoot);
        }
 ...
 */
function collectWebpackChunks(dir, swPath, hoisted) {
    let swSrc;
    try {
    ...
    for (const abs of walkScripts(dir)) {
        ...
        if (!m || !CHUNK_PREFIX_RE.test(src.slice(0, m.index)))
            continue;
        const open = m.index + m[0].length - 1; // the push's "("
        const end = matchBalancedParen(src, open);
        if (end < 0)
            continue;
        if (pushArrayElementCount(src.slice(open + 1, end - 1)) !== 2)
            continue;
        if (!CHUNK_SUFFIX_RE.test(src.slice(end)))
            continue;
        chunks.push(rel);
    }
    return chunks.sort();
 ...
 */
export function pushArrayElementCount(s) {
    let start = 0;
    ...
    const at = walkCode(s, start, (c) => {
        if (c === "[" || c === "(" || c === "{")
            ...
            depth--;
            if (depth === 0)
                return true; // closed the outer array
        }
        else if (c === "," && depth === 1)
            count++;
        return false;
